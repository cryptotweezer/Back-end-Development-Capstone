...
def songs(request):
    try:
        res = req.get(f"{os.getenv('SONGS_URL')}/song", timeout=5)
...
def photos(request):
    try:
        res = req.get(f"{os.getenv('PICTURES_URL')}/picture", timeout=10)
        if res.status_code == 200:
            photos = res.json()
            print("Fotos recibidas:", len(photos))
        else:
            print("Error en la respuesta:", res.status_code)
            photos = []
    except Exception as e:
